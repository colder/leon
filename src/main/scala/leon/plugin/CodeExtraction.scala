...
trait CodeExtraction extends Extractors {
  ...
  class Extraction(unit: CompilationUnit) {
    ...
    private def extractTree(tr: Tree): Expr = tr match {
      case Block(es, last) =>
        val recEs   = es.map(extractTree _)
        val recLast = extractTree(last)

        LeonBlock(recEs, recLast).setType(recLast.getType)

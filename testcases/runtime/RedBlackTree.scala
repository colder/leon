...
object RedBlackTree {
  ...
  def addSynth(x: Int, t: Tree): Tree = choose {
    (res: Tree) => redNodesHaveBlackChildren(t) && blackBalanced(t) && content(res) == content(t) ++ Set(x) && redNodesHaveBlackChildren(res) && blackBalanced(res)
  }
  ...
  def main(args: Array[String]) {
    val tree = add(3, add(2, add(1, add(0, Empty()))))

    val res = addSynth(4, tree)
  }
}
